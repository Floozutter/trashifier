...
const int ENCODERPOS_LEFT = +120;
const int ENCODERPOS_RIGHT = -140;
...
void loop()
{
  ...
  //waitForLow(buttonPin);
  
  // ask Python for command
  Serial.println('s');
  delay(2000);
...
void waitForLow(int pin)
{
  ...
  while (wait)
  {
    if (digitalRead(pin) == LOW) { wait = false; }
  }
}
